        ...
        return (
          <div key={s.stage}>
            <div className="mb-1 flex items-center justify-between text-sm">
              <span className="font-medium text-ink">{labelMap[s.stage] ?? s.stage}</span>
              <span className="text-ink-muted">
                <span className="font-semibold text-brand-navy">{s.count.toLocaleString()}</span>
                {conversion !== null && (
            ...
            <div className="h-7 overflow-hidden rounded-md bg-surface-muted">
              <div
                className="flex h-full items-center justify-end rounded-md bg-brand-greenInk px-2 text-xs font-semibold text-white transition-all"
